...
//$OPT --java-heap 4096
//$OPT -d /home/gabriele/TissueMaps_ExeDir/result_170315_161220_hippo_4_1_20x_3d
...
object pipeline {
	val prepro_pipeline_path = INPUT(path="/home/gabriele/sourceCode_repo/3DprePro_pipeline")
	prepro_pipeline_path._execute = "once"
	val predProb_pipeline_path = INPUT(path="/home/gabriele/sourceCode_repo/3DprePro_pipeline/network")
	...
	val dataset_folder = INPUT(path="/data/project_data/170315_161220_hippo_4_1_ROI")
	...
	val norm_values = BashEvaluate(var1=prepro_pipeline_path, var2=img_CSV, script="""source activate pgm_pipeline
				python -u @var1@/norm.py @var2@ 99 $( getmetadata "custom_cpu" ) @out1@ 50000
				source deactivate""")
	norm_values._custom("cpu") ="1"
	norm_values._custom("partition") = "-p p16"

	// REGISTRATION
	val registration = BashEvaluate(var1=prepro_pipeline_path, var2=img_CSV, param1="8", param2="rigid", param3="Nuclei", param4="512", param5="4", script = """ source activate pgm_pipeline
				python -u @var1@/mainReg.py @var1@ @folder1@ @var2@ @param1@ @param2@ @param3@ @param4@ @param5@ "noBspline"
				source deactivate""")
	...
	for ((k,f)<-iterArray(allTiles).take(1)) { //take tiles idecies
		...
		//main_files.retain((k1,v) => "%s".format(k1) == "0_1_tif") /////
		for ((r,file)<-main_files){
			withName(r){
				...
				val prePro = BashEvaluate(var1=prepro_pipeline_path, var2=tile, var3=norm_values.out1, script="""source activate pgm_pipeline
						python -u @var1@/main_3D.py @var1@ @var2@ @folder1@ @out1@ @out2@ $( getmetadata "custom_cpu" ) 0.05 @var3@ @out3@ 50 @folder2@/candidates_max.h5
						source deactivate """) 
				prePro._custom("cpu") ="4"
                                prePro._custom("partition")="-p p32"
				//prePro._execute = "once"

				// POSTPROCESSING
				val predProb = BashEvaluate(var1=predProb_pipeline_path, var2=prePro.out1, param1="0.3", script="""source activate pgm_pipeline
						python @var1@/get_proba_DNN.py @var1@ @var2@ @out1@ @param1@
						source deactivate""")
				...
				//		python @var3@/Q_thresh3D.py @var1@ @var2@ @var5@ @var4@.npy @var6@ @out1@ @out2@ @out3@
				//		source deactivate""")
				...
				//val predProb_pass2 = BashEvaluate(var1=predProb_pipeline_path, var2=unexp.out3, script="""source activate pgm_pipeline
                                ...
                                //                python -u @var1@/mainSSBA.py @var1@ @var2@ @out2@ @out1@ $( getmetadata "custom_cpu" ) 2.5 9 @out3@ prior @var3@ @var4@ @var5@ @var6@
                                //                source deactivate""")
