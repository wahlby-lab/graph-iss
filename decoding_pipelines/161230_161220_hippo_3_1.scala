...
//$OPT --java-heap 4096
//$OPT -d /home/gabriele/TissueMaps_ExeDir/result_161230_161220_hippo_3_1
...
object pipeline {
	...
	val dataset_folder = INPUT(path="/data/project_data/161230_161220KI_3-1")
	dataset_folder._execute = "once"
	val tagList = INPUT(path="/home/gabriele/sourceCode_repo/graph-iss/data/tagList_84-gene.csv")
	...
	mode_percent._custom("cpu") ="2"
        ...
        // REGISTRATION
        val registration = BashEvaluate(var1=prepro_pipeline_path, var2=img_CSV, param1="8", param2="translation,rigid", param3="Nuclei", param4="DO", script="""
                              eval "$(command conda 'shell.bash' 'hook' 2> /dev/null)"
                              conda activate pgm_pipeline
	...
	for ((k,f)<-iterArray(allTiles).take(1)) { //take tiles idecies
		...
		//main_files.retain((k1,v) => "%s".format(k1) == "17_10_tif") /////
